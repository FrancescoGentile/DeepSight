...
class Criterion(_Criterion[Output, Annotations], Configurable):
    ...
    def losses(self) -> Iterable[str]:
        ...
        for layer_index in self.layer_indices:
            losses.append(f"suppression_loss_{layer_index}")
            losses.append(f"classification_loss_{layer_index}")
    ...
    def compute(
        ...
        gt_interactions, gt_labels = _batch(annotations, output[-1].num_nodes)

        losses = {}
        for idx, layer_output in enumerate(output):
            matched = layer_output.indices.unsqueeze(1) == gt_interactions  # (E, E', 2)
            ...
            if layer_output.interaction_mask is not None:
                cls_loss = cls_loss.masked_select(
                    ~layer_output.interaction_mask[matched_pred]
                )
