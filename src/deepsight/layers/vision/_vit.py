...
from deepsight.typing import Tensor, str_enum
...
class PatchEmbedding(nn.Module):
    ...
    def forward(
        self, x: Tensor[Literal["B C H W"], float] | BatchedImages
...
class SelfAttention(nn.Module):
    ...
    def forward(
        self,
        x: BatchedSequences | Tensor[Literal["B N D"], float],
        mask: Tensor[Literal[" B 1 1 N"], bool] | None,
    ) -> Tensor[Literal["B N D"], float]:
        B, N, D = x.shape  # noqa
...
class LayerScale(nn.Module):
    ...
    def forward(
        self, x: Tensor[Literal["B N D"], float]
    ) -> Tensor[Literal["B N D"], float]:
